...
public final class JsonOutputParser {
     ...
     */
    public static List<IssueRespDTO> parseIssues(String response) {
        try {
    ...
    /**
     * 将 Map 转换为 IssueRespDTO 对象。
     */
    private static IssueRespDTO mapToIssue(Map<String, Object> map) {
        IssueRespDTO issue = new IssueRespDTO();
        String severityStr = (String) map.get("severity");
        if (severityStr != null) {
            try {
                issue.setSeverity(Severity.valueOf(severityStr.toUpperCase()));
