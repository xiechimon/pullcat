...
当任务涉及前端页面、组件或视觉设计时，默认按下面规则选择 skill：

- 新建页面、重做页面、实现落地页、仪表盘、组件视觉改造：`frontend-design`
- 优化现有页面的视觉层级、布局、交互、可用性、响应式体验：`impeccable`
- 制定整体设计语言、配色、字体、组件风格体系：`ui-ux-pro-max`

如果任务同时包含“先评审再改造”，先用 `impeccable`，再用 `frontend-design`。
...
### 注释约束
允许给字段补充简短 Javadoc，包括基础包装对象。 但中文短句注释末尾不要加句号。 应该这样写：
```java
/**
 * 是否成功
 */
