...
const BASE_CLS = 'rounded-xl p-16 flex flex-col items-center justify-center border-2 border-dashed border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30 space-y-5 transition-all duration-300 animate-fade-in'
...
export function LoadingPlaceholder({ task }: LoadingPlaceholderProps) {
  ...
  if (task.status === 'PENDING') {
    return (
      <div className={BASE_CLS}>
        <div className="h-10 w-10 text-gray-300 dark:text-gray-600 flex items-center justify-center text-3xl animate-pulse">○</div>
  ...
  if (task.status === 'FAILED') {
    return (
      <div className={BASE_CLS}>
        <span className="text-2xl text-red-400 dark:text-red-600 animate-fade-in" aria-label={`${task.label} 分析失败`}>✕</span>
      </div>
    )
  }
